...
@section('content')
    ...
    <section class="m-10 pt-16">
        <div class="pb-5 min-h-full" style="font-family: 'Quicksand', sans-serif;">
            <h4><a href="{{ route('beranda') }}">Home</a> > <a href="{{ route('user.category-puisi') }}">{{ $post->kategori }}</a> > <a href="#" style="color: rgba(96, 165, 250, 100);">{{ $post->title }}</a></h4>
        </div>
        <a href="#" class="inline-block text-sm px-10 py-5 leading-none bg-blue-400 text-black rounded-2xl hover:shadow-lg hover:text-teal mt-4 lg:mt-0 transform hover:scale-105 motion-reduce:transform-none" style="font-family: 'Quicksand', sans-serif;">{{ $post->kategori }}</a>
        <!-- component -->
        ...
        <div class="flex flex-row w-full">
            <div class="w-3/4">
                <h3 class="pt-16 pb-5" style="font-family: 'Quicksand', sans-serif; font-size: 24px; font-weight: normal;">{{ date('F d, Y', strtotime($post->schedule)) }}</h3>
        ...
        <div class="rounded-3xl p-10 md:p-15 lg:p-20 bg-blue-200">
        ...
        <div class="flex flex-row pt-14 justify-between" style="font-family: 'Poppins', sans-serif;">
            <div class="w-auto text-left">
                <a class="flex-auto text-black hover:text-blue-500" href="#">
